...
| **15 张 · 新玩法** | 45 张（去大小王、三个2、三个A、一个K） | 弃 15 底 → 两家各 15 张 | ✅ **pdk45 深度 AI**（`/ai15` 平台流，与 16 张同套协议，不降级） |
...
- 选择结果记在 `localStorage.pdk_deck_spec`，下次打开沿用；15 张下的「链路自检」走 pdk45 专用断言（§下方），「服务端复盘」仍仅覆盖 16 张局。

### 15 张玩法数据分区（与 16 张彻底隔离）

- **编号与目录**：`A/H/E` = 16 张（A 人机 / H 真人房 / E 对外 API，`data/replays`·`online`·`external`）；
  **`B` = 15 张**（`data/replays15`）。编号前缀即玩法标识，物理目录分离，互不串档。
- **存档**：15 张终局由前端 `POST /replays/save` 上传（`deck:15`），**服务端权威结算**
  （`settle.py`，与 app.js `settle()` 同规则）并分配 B 编号；顶栏显示「本局 B####」。
  16 张仍由桥在服务端落盘，两条写入路径互不重叠（接口拒绝 `deck!=15`）。
- **读取**：`/replays/list|get|stats` 支持 `?deck=all|16|15`；列表行带 `deck` 字段与玩法徽标；
  跨档查询返回「找不到」而不是串到另一档。
- **点评（经验填写）**：每条点评记录带 `deck` 字段，读取按牌档过滤；15 张复盘可正常写点评。
- **复盘**：15 张支持逐手复盘（手牌/出牌用 bot id 口径，与 16 张同渲染）；16 张专用的
  「AI 解析这手」（走桥）在 15 张档隐藏。
- **渲染修复**：牌面 id 反推公式对 A/2 不成立的老问题已修（此前 ♠2 显示为 A♦、♥A 显示为 A♠）。

### 16 张引擎的 2026-09-24 部署（H5 已生效，版本行可见）

- **批次111 / B2**：`PROD_SOLVER_KW.mid_solve_chunk = 128` —— 求解器分块，治"批内预算饿死池尾候选"
  （纯正确性改动）。H5 版本行显示「B2 分块 128」；回退=改 0 并重启 `paodekuai-ai paodekuai-api`。
- **顶牌纪律 `single_top_rule`**：桥按局号分流 A/B，线上 drop-in `PDK_AB_ALL_B=1` → **100% 局**
  打开（依据 +2.03pt / 净分 +0.253，16 种子 CI 排除 0）。版本行显示「顶牌纪律 100%」，
  自检显示 B/A 臂计数与"无声 A/A"计数（`AB_STAT`）。
  ⚠ **权衡**：100% 铺开后真人对照臂消失 ⇒ 之后无法再在真实流量上量这条规则的效果
  （历史口径带 ~21pt 台架伪影，不可用作基线）。要保留测量能力就把 drop-in 的
  `PDK_AB_ALL_B` 置 0（=50/50 灰度）并重启。
- **口径统一 #3**：`PDK_CFG_FROM_SESSION=1` —— agent 用**当局规则**跑内部模拟（此前与
  CGame 有 2/4 条规则相反）。版本行显示「当局规则 ✓」。
- **桥的形状校验**：`pdk/trickguard.py`（引擎侧唯一实现）已装进生产树；`/api/decide` 对
  **不可能存在**的 trick 描述返回 400（连对 len 传张数的"看起来像 AI 变保守"那类坑）。
- **长决策加固（H5）**：B2 后单决策实测最长 ~8.7s；`aiMove` 加**防重入**（`S.aiPending`），
  回合看门狗跳过"决策在途"，避免把长决策误判成卡死而重复派发。

### 15 张深度 AI（pdk45，已接入）

15 张的出牌决策与「提示」统一走 **pdk45 全量服务**（pdk-ai 仓库 `pdk15` 分支，提交 97ed3f4）：

- **服务**：腾讯云 `paodekuai-ai15`（:8765），`server.py --deck-profile pdk45`；生产槽位
  `ckpt/policy_a2c_final56.pt` = pdk45 冠军网 **ft2best**（a2c56 · hidden 256，
  md5 `457425bb315f68084dda5dfd917665fc`；守卫脚本拒绝 std48 权重顶替）。
- **接入形态**：平台流（API_PDK45.md §5）——网页持局，每手 `POST /ai15/api/decide`
  （web 服务反向代理 → 127.0.0.1:8765），传 `my_hand / opp_n / trick / history`，
  决策视角（人/AI/热座当前座位）在 history 里重映射为「调用方=0」。
- **牌 id 映射**：前端 15 张 id ↔ pdk45 id——`0..42`（3~Q 与 ♠♥♣K）同号；
  前端 `44`(♥A)→`44`、`47`(♠2)→`48`。前端 15 张副 = 48 张副去掉 ♦K(43)、♣A(45)、♦A(46)。
- **无降级**：决策/提示失败一律暂停报错（`aiError`，绝不退回本地贪心）；AI 返回的着法
  必须在本地合法集内，否则报「引擎与前端失步」并暂停。接入前断言 API_PDK45 §2 全部字段
  （`deck_profile=pdk45`、45/15/15、c_core、a2c56·256、prod_agent、hybrid）+ `/api/selftest`。
- **大厅自检（15 张档）**：按 §2 断言 `/ai15/api/health` + `/api/selftest` 金丝雀。
- **时延**：2 核腾讯云实测 decide P50≈1.3s / P95≈2.1s（预算 6s）；E2E 5 局全正常终局。
...
- 每颗未被压掉的炸弹**由输家多付 10 分给赢家**（不论谁炸的；被更大的炸弹直接压掉则不计分）。
  > 2026-09-24 用户拍板改口径：旧口径「炸弹持有者向被炸方收 10 分（与胜负无关）」会让输家自己的炸弹
  > 把底分抵成 0（A1737：AI 胜、人类剩 10 张 → 0 分）。**只影响新局，历史棋谱保留当时算出的分数**。
