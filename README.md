...
| 区块 | 内容 | 说明 |
|---|---|---|
| 🃏 手牌大概是什么 | `rank_prob` / **`rank_cnt_p`** / `rank_exp` / **`opp_most_likely`** / `top_hands` | 「有没有」（P≥1 张）+ **「到底几张」**（`rank_cnt_p` 张数分布：没有/1 张/2 张…各多少概率，众数加粗）+ 最可能的一手牌（`opp_most_likely`，点数→张数）+ 前几种具体构成；**按概率从高到低排序** |
| 🧮 记牌台账 | `ledger` | 各点数"未露面"张数（总量 − 我手牌 − 双方已出）+ 该点数概率，圆点阵一眼看剩几张；**格子同样按概率降序**，已断张自动沉底 |
| 🔍 推断链 | `facts` | 模型从对手动作推出的事实，**每条带实测命中率**（领出规则 77~91%、过牌硬约束、断7/断10、tell），可逐条校验该不该信 |
...
张数口径（上游 `8325d96` 起）：`rank_prob` 只回答“是否持有”，要回答“到底几张”看 `rank_cnt_p`（如 `{"K":{"0":0.69,"1":0.21,"2":0.10}}` = 69% 没有 K / 21% 恰好 1 张 / 10% 恰好 2 张）；份额 <0.5% 的档位不列，只有 1 张的点数（如 ♠2）天然只有 0/1 两档。
