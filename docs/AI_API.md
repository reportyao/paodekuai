...
| 0 | 单张 SINGLE | — | 点数 | 1 |
| 1 | 对子 PAIR | — | 点数 | 2 |
| 2 | 连对 PAIRS | ≥2 连对（如 7788） | **最大**点数 | **对数**（3 对=3，不是张数） |
| 3 | 三张 TRIPLE | 仅末手可出/接 | 点数 | 3 |
| 4 | 三带二 TRIPLE2 | 555+3+4 → `[4, 2, 5, 0]` | 三张点数 | 5（张数） |
| 5 | 三带一 TRIPLE1 | 仅末手 | 三张点数 | 4（张数） |
| 6 | 飞机 AIRPLANE | 三张连（需变体开关） | 最大点数 | **三张组数**（333444=2，不是张数） |
| 7 | 顺子 STRAIGHT | ≥5 连张，不含 2 | **最大**点数 | 张数 |
| 8 | 炸弹 BOMB | 3333–KKKK | 点数 | 4 |
| 9 | 四带三 FOUR3 | 需变体开关 `four3` | 四张点数 | 7（张数） |

> ⚠️ 四个容易踩的点：
> 1. **`main` 对顺子/连对是"最大点数"**（顺子 3-4-5-6-7 → `main=4` 即 7）。解释文案里"顺子起X"用的是同一字段，属上游措辞问题。
> 2. **`len` 的含义随牌型而变**：只有连对（对数）与飞机（三张组数）是"组数"，其余都是张数。
>    把连对的 `len` 传成张数（7788 传 4）**不会再静默**：服务端用引擎自己的 `classify`
>    反查该四元组能否由某手牌成立，不成立直接 **HTTP 400** 并给出正确写法。
>    （在这道防线之前，错的 `len` 会让引擎按"四连对"去找解 —— 手上没有就直接过牌，
>    `legal_count=1`，看起来像"AI 变保守了"。）
> 3. `nc` 是内核附加计数（引擎 `classify` 输出：炸弹=1、飞机=组数，其余=0），**不参与任何
>    判定**（仅并入局面哈希），传 `0` 或引擎输出都等价 —— 不必为它纠结。
> 4. **推荐做法：调用方不要自己构造 `trick`** —— 让服务端产出（见 §3.5 `/v1/legal`），
>    或原样回传上一次服务响应里的值。顺子/连对/飞机不含 2（`main` 最大 `11`=A）。
...
```json
{"red10": true, "four3": false, "nobomb": false, "sanzhang": false}
```
...
| `nobomb` | `false` | 炸弹不可拆开当作其他牌型打出。**默认已改为可拆**（网页版已移除该开关）；旧棋谱/兼容调用仍可显式传 `true` |
...
5. 炸弹可压任意牌型；炸弹之间比点数；**炸弹默认可拆开**当其他牌型打出（兼容键 `nobomb=true` 可恢复不可拆）
